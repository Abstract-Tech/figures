...
class BaseStatCard extends Component {
  ...
  calculateComparisonValue = () => {
    if (
      this.props.compareToPrevious &&
      !this.props.singleValue &&
      this.state.valueHistory.size
    ) {
      if (this.props.dataType === "percentage") {
        this.setState({
          comparisonValue: `${((this.state.mainValue -
            this.state.valueHistory.getIn([
              this.state.valueHistory.size - 2,
              "value",
            ])) *
            100).toFixed(2)}%`,
        });
  ...
  initialDataSet = () => {
    if (this.props.fetchValueFunction && this.props.fetchDataKey) {
      this.props.fetchValueFunction(this.props.fetchDataKey).then(response => {
        if (response.error) {
          console.log(
            `Error fetching the ${this.props.cardTitle} data: ${response.error}`
          );
        } else {
          ...
          this.setState(
            {
              mainValue: this.props.mainValue
                ...
                : responseData.getIn(
                    [this.props.fetchDataKey, this.props.dataMainValueKey],
                    this.state.mainValue
                  ),
              valueHistory: this.props.valueHistory
                ? this.props.valueHistory
                : responseData.getIn(
                    [this.props.fetchDataKey, this.props.dataHistoryKey],
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (this.props !== nextProps) {
      this.setState(
        {
          mainValue: nextProps.mainValue
            ? nextProps.mainValue
            : this.state.mainValue,
          valueHistory: nextProps.valueHistory
            ? nextProps.valueHistory
            : this.state.valueHistory,
        },
        () => {
          this.calculateComparisonValue();
  ...
  render() {
    return (
      <div
        className={cx({
          "stat-card": true,
          "span-2": this.state.cardWidth === 2 || 4,
          "span-3": this.state.cardWidth === 3,
        })}
      >
        <div className={styles["main-content"]}>
            ...
            {this.props.singleValue
              ...
              : <span className={styles["current-data"]}>
                  {this.props.dataType === "percentage"
                    ? (this.state.mainValue * 100).toFixed(2)
                    : this.state.mainValue}
                  {this.props.dataType === "percentage" && "%"}
                </span>}
            {this.props.compareToPrevious &&
              ...
              <div className={styles["previous-comparison"]}>
                ...
                <span className={styles["comparison-text"]}>
                  since last month
                </span>
              </div>}
          </div>
          {this.props.enableHistory && !this.props.singleValue
            ? <button
                onClick={this.historyToggle}
                className={styles["history-toggle"]}
              >
                {this.state.historyExpanded ? "hide history" : "see history"}
              </button>
            : <span className={styles["history-toggle-faux"]} />}
        </div>
